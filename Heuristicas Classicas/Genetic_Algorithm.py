...
class GA:
    ...
    def calculate_fitness(self):
        ...
        total_dist += sum(self.price[self.gene[i] - 1][self.gene[i + 1] - 1] for i in range(len(self.gene) - 1))
    ...
    def crossover(self, other):
        child = self.gene[:]
        cut_point1, cut_point2 = sorted(random.sample(range(1, len(self.gene)), 2))
    ...
    def multipoint_crossover(self, other, points=2):
        ...
        for i in range(len(indices) - 1):
            if i % 2 == 1:
                child[indices[i]:indices[i+1]] = other.gene[indices[i]:indices[i+1]]
        return GA(child, self.price, self.tabu_list)
    ...
    def two_opt(self):
        ...
        while improved:
            ...
            for i in range(len(self.gene) - 2):
                for j in range(i + 2, len(self.gene)-1):  # Adjust here
                    if self.two_opt_gain(i, j) < 0:
    ...
    def calculate_fitness_parallel(individual):
        total_dist = sum(individual.price[individual.gene[i] - 1][individual.gene[i + 1] - 1] for i in range(len(individual.gene) - 1))
        return total_dist
    ...
    def evolve(cls, population, generations=100):
        for generation in range(generations):
            ...
            print("Generation: ", generation, " Best path:", population[0].gene, "\n Cost: ", round(population[0].fit), "\n")
